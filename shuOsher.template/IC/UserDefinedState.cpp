 ...
 ************************************************************************/
/*
 * Modified by Xuhui Zhou, 2025:
 * configured restart input for EnKF cases.
 */
...
//-------------------------------------------------------------------------
template<typename T>
T Interpolate(vector<T> val, vector<bool>& valid, Vec3D &coeff); //!< interpolation function used in this file
//-------------------------------------------------------------------------
...
MyStateCalculator::GetUserDefinedState(int i0, int j0, int k0, int imax, int jmax, int kmax,
                                       Vec3D*** coords, Vec5D*** v, double*** id, vector<double***> phi)
{
  // This function currently assumes AT MOST one level set function (phi). Extension is straighforwrad.

  // --------------------------------------------------------------
  // USER INPUTS
  // --------------------------------------------------------------
  // 1. Restart solution file.
  //    Note: This solution file should at least contain density, velocity, pressure, and mateiral ID.
  //          If it involves a level set function (i.e., "phi"), it should be included as well.
  std::string filename = "IC/solution_0000_analysis.vtr";
  // 2. Relate material IDs with phi. (If the problem does not involve level set, set both to 0)
  int PHI_NEG_MATID = 0;
  int PHI_POS_MATID = 0;
  // 3. Set material ID for inactive material region.
  int INACTIVE_ID = 1; //!< problem-specific, usually equal to the number of active materials.
  // 4. Set a tolerance (dimension: length) for constant extrapolation outside the grid of
  //    the above-specified solution. This should be small compared to mesh resolution ---
  //    mostly for dealing with round-off errors (i.e., ``sealing'' gas in numbers).
  double tol_extrap = 1.0e-5;
  // --------------------------------------------------------------
  // ENDO OF USER INPUTS
  // --------------------------------------------------------------
  ...
  assert(nVar>=4); //'density', 'velocity', 'pressure', 'materialid', 'levelset0', 'temperature', 'laser_radiance', ..
  ...
  auto levelset = (nVar>4 && (string)grid->GetPointData()->GetArray(4)->GetName() == "levelset0") ?
                  grid->GetPointData()->GetArray(4) : NULL;
...
//------------------------------------------------------------
//Interpolation function used above
template<typename T>
...
//------------------------------------------------------------
// The class factory (Note: In most cases, they do NOT need to be modified.)
extern "C" UserDefinedState* Create() {
  return new MyStateCalculator;
}
